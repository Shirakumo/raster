    ...
    (let ((mask (aref source (+ si i))))
      (when (< 0 mask)
        (let* ((color (funcall sampler i j))
          ...
          (setf (aref target (+ 1 ti i)) (alpha-blend (ldb (byte 8  8) color) (aref target (+ 1 ti i)) alpha))
          (setf (aref target (+ 2 ti i)) (alpha-blend (ldb (byte 8 16) color) (aref target (+ 2 ti i)) alpha))
          (setf (aref target (+ 3 ti i)) (alpha-blend 255 (aref target (+ 3 ti i)) alpha)))))))
                   ...
                   do (let* ((x (coordinate (+ j sx ox)))
                        ...
                        (when (<= 0 sdf)
                          (let* ((ti (+ ti (* 4 i)))
                            ...
                            (setf (aref target (+ 1 ti)) (alpha-blend (ldb (byte 8  8) color) (aref target (+ 1 ti)) alpha))
                            (setf (aref target (+ 2 ti)) (alpha-blend (ldb (byte 8 16) color) (aref target (+ 2 ti)) alpha))
